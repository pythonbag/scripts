#!/usr/bin/python
#
# pull the sportability.com team info
#
import sys
import csv
import re
...
def pullContact(row,league,team):
    #print row
    for key in row:
    ...
    #return data
    return contact
def csv_reader(filename,league,team):
     fall2015 = []
     names = []
     reader = csv.DictReader(open (filename))
     for row in reader:
         data = pullContact(row,league,team)
         if len(data) > 0:
             if data["phone0"] != "":
                 print data["phone0"]
             if data["phone1"] != "":
...
if __name__ == "__main__":
    csv_path = "./2015fallayso2.csv"
    csv_reader(csv_path,sys.argv[1],sys.argv[2])
